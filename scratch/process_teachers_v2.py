...
for item in raw_data:
    ...
    # Add subject_code to specializations if it's not N/A
    if item['subject_code'] != 'N/A':
        specs.append(item['subject_code'])
    
    for full_name in raw_names:
        # Clean sub-names recursively in case of nested "and" (unlikely here but safe)
        sub_names = clean_name(full_name) if ' and ' in full_name else [full_name]
        for sub_name in sub_names:
            name_parts = sub_name.strip().split(' ')
            ...
            processed.append({
                ...
                "last_name": last_name,
                "email": email,
                "specializations": list(set(specs)), # Unique list
                "max_periods_per_week": 25,
                "is_active": True,
                "primary_subject_code": item['subject_code']
            })
...
print(f"Processed {len(processed)} teachers with subject codes.")
